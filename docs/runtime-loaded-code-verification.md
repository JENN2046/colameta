...
`get_runtime_version_status` now reports whether the running ColaMeta MCP/Web process appears to be serving code loaded from the current project checkout or from stale module sources. This is read-only proof only. It does not restart, reload, kill, apply, fetch, pull, push, tag, release, mutate service lifecycle state, mutate executor workflow state, mutate config, or mutate Git remote state.

## What Is Verified

The status result compares two independent pieces of evidence:
...
This verification does not claim full Git worktree cleanliness. It does not scan every tracked or untracked file, and it does not prove that files outside loaded `runner.*` modules are clean. It only proves whether the loaded runtime HEAD matches the current checkout HEAD and whether captured loaded module fingerprints still match their current source files.
