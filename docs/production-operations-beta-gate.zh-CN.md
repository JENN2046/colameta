...
`loaded_runtime_head` 与 `expected_head` 一致。对于通过 non-editable pip install
启动的 packaged stable service，如果 `loaded_runtime_head` 为空，也可以用
...
loopback HTTPS URL 会直接 rejected，不能满足 remote preflight。
