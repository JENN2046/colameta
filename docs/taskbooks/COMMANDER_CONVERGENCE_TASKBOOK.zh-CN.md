...
commander_convergence_taskbook_chinese_companion:
  ...
  source_sha256: d1776e0208e2d0562641ed96a50148e2c7eb7b17d68a12cdd405243da5586da7
...
后续 PR 审查发现：即使 canonical decision 已安全允许 `start_new`，
`stale_session_resume_forbidden` 仍被错误投影为通用 hard blocker。审查修复现已将该原因仅保留
在 `resume_blockers`，并新增 agent-dispatch precheck 回归测试。最终顺序验证与复审证据据此
重新绑定。
