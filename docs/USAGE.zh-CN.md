...
最近一次请求状态，例如 pending、updated、requested 或 blocked。如果 ChatGPT Apps
direct tool call 失败并退回 MCP bridge，卡片会保留一段简短错误摘要和 fallback 请求状态。
...
保留，但不会再被当成当前成功证据。
当结果是 `updated` 或 bridge `requested` 时，每个 action 的 `next_refresh_actions` 和
