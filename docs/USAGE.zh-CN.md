...
每个推荐动作都使用稳定 action 形状：`action_id`、`label`、`mode`、`status`、
`tool` 或 `runbook`、`arguments`、`required_scope`、`requires_preview_confirm`、
`requires_explicit_confirmation`、`side_effects`、`authority_boundary` 和
`result_contract`。`mode=commit` 表示调用该工具会写入本地项目状态，必须显式确认；
