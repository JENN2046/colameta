...
For a stable ChatGPT HTTPS remote MCP endpoint, follow [Remote HTTPS MCP Service](docs/remote-https-mcp-service.md); that path requires OAuth and a stable HTTPS `/mcp` URL.
...
Submission evidence that explicitly remains draft is revised through `manage_submission_evidence_revision`: preview binds a manifest-owned Markdown path to current/proposed digests, apply rechecks both the file and manifest before an atomic replacement, and the ready field remains false until separate human review. The Web Console's **EVIDENCE** workspace exposes bounded source text only through authenticated local read routes. For `mcp_tool_info`, `security_review`, and `metadata_snapshot`, the editor can load a current-facts auto draft through the same read-auth boundary; this only replaces the browser draft and still requires operator correction because unfinished declarations remain blocked. The editor invalidates a revision preview whenever content changes; for ready review, it requires the operator to open and explicitly confirm every ref for one key, binds all reviewed digests and the manifest into a short-lived preview, then uses a one-use dangerous confirmation to mark only that key ready. Both write flows refresh Product Console status after success.
The Web Console **INBOX** also closes the stable-promotion artifact receipt flow without granting deployment authority: status and write calls require Web read authentication, preview computes a summary-only exact-HEAD manifest from the Git object database, and apply requires both the write guards and a one-use dangerous confirmation before persisting the immutable runtime receipt, then recomputes and verifies that receipt. Worktree content is never included in the candidate manifest, and this flow cannot replace or restart stable, push, release, or deploy.
