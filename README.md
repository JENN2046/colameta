...
The Web Console **INBOX** also closes the stable-promotion artifact receipt flow without granting deployment authority: it authenticates status reads, computes a summary-only exact-HEAD manifest preview from the Git object database, requires a one-use dangerous confirmation before persisting the immutable runtime receipt, then recomputes and verifies that receipt. Worktree content is never included in the candidate manifest, and this flow cannot replace or restart stable, push, release, or deploy.
