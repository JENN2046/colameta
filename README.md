...
- **项目运行目录**：`.colameta/` 保存 plan、state、prompts、runtime、logs、reports、workflow 记录和执行器会话。历史 `.mvp-runner/` 只作为旧数据迁移或审计输入，不作为当前项目运行入口。
...
当前项目内 Runner 元数据入口统一为 `.colameta/`。历史 `.mvp-runner/` 只允许作为旧数据迁移或审计输入被显式识别，不作为当前产品口径。
