# ColaMeta 项目主任务书 v1.1 Candidate 1 中文 Companion

```yaml
chinese_companion:
  source_document: PROJECT_MASTER_TASKBOOK.v1.1-candidate.1.md
  source_sha256: 40c6af59e10ae488c58230e5a29d1348824101485fae86daf9fff1d3d019d528
...
本中文文件是 `PROJECT_MASTER_TASKBOOK.v1.1-candidate.1.md` 的中文 companion。它不替代英文源文件的
...
id: colameta_master_taskbook_v1_1_candidate_1
canonical_name: Master Taskbook
project: ColaMeta
version: v1.1-candidate.1
status: discussion_draft
candidate_id: colameta-master-v1.1-p1-contract-convergence-candidate.1
canonical_path: PROJECT_MASTER_TASKBOOK.v1.1-candidate.1.md
...
- 当前候选版本是 `v1.1-candidate.1`，用于同时修正三个 P1 合同问题。
...
- 它从 raw SHA `1b2d787465eef52a177f4716ea7495704e03c390ce6f0e3d26ca16b360688e34`
  的现行 Master 精确快照派生而来；
- 现行 Master 继续保持该 hash 的 hash-specific freeze-candidate review status；
- 本候选在单独激活前不替换现行 Master，该 review status 也不转移到本候选；
- 本候选的新 raw SHA 与 canonical freeze hash 必须另行进入 hash-specific review。
...
当前源文件内部状态仍是 `discussion_draft`。三个 P1 合同修正已经进入候选稿，
但候选仍等待新的 hash-specific review；推荐 review decision 仍是 `CONTINUE_DISCUSSION`。
...
本候选已经写入三项 P1 修正：canonical payload/freeze hash 可机械复现；
ReviewDecision 的 transition 字段按 `resulting_action` 条件必填；GateEvent 的
transition 字段按 `event_type` 条件必填。这里的“已写入”不等于 review closure 或状态提升。
...
- PLAN_ADJUST / ABORT 只允许 Commander decision request 或 no action，禁止嵌入 GateEvent
  transition 字段；
...
3. 审查新 `v1.1-candidate.1` 的三个 P1 合同修正、raw SHA 与 canonical freeze hash。
4. 只有在新的单独 hash-specific Commander authorization 且全部 activation requirements 满足后，才考虑从
