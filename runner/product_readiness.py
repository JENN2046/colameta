...
def build_product_readiness_packet(
    ...
    *,
    ...
    return {
        ...
        "safe_next_action": _safe_next_action(status, primary_blocker),
        "chatgpt_app": {
            "status": status,
...
def build_chatgpt_connection_packet(
    ...
    **kwargs: Any,
    ...
    readiness = build_product_readiness_packet(project_root, **kwargs)
...
def _safe_next_action(status: str, primary_blocker: dict[str, Any] | None) -> dict[str, Any]:
    if status == READY:
        return {
