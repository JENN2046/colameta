...
每个读取或分派边界都会把这些事实捕获成一份 provider-aware
`continuation_snapshot`。公开的 `snapshot_id` 用来证明 Web、Analyze、Thin-loop、Commander 和
执行器 guidance 投影的是同一次捕获；后续新请求通常会得到新的 ID。私人 session identity
不会出现在公开 snapshot 中。

执行器写操作还使用项目级共享/独占 POSIX operation lease。Lease 直接持有既有项目根目录的
directory descriptor，不会在项目里创建 lock file。已有操作占用 lease 时返回
`PROJECT_OPERATION_BUSY`；当前环境无法安全建立 lease 时返回
`PROJECT_OPERATION_LEASE_UNAVAILABLE`，两者都 fail closed。它要求 POSIX `flock`、项目根目录
属于当前有效服务用户，并且没有 group/world write 位（`mode & 0o022 == 0`）。不要通过删除
runtime/session 数据或放宽权限绕过错误：前者等待当前操作结束，后者先修正平台、owner 或权限
不匹配，再重试。
...
Runtime loaded-code verification 会只读报告已加载模块是否可能落后于当前 checkout 或本地源码；见 [Runtime Loaded-Code Verification](docs/runtime-loaded-code-verification.md)。
